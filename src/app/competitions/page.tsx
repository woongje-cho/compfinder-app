...
export default function CompetitionsPage({
  ...
  return <CompetitionsContent searchParamsPromise={searchParams} />;
}
...
async function CompetitionsContent({
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-3xl font-bold text-zinc-50">Competitions</h1>
      ...
      <div className="mb-6 flex gap-2">
        <RegionTab region={undefined} label="All" count={all.length} active={!activeRegion} params={params} />
        {(Object.keys(REGION_CONFIG) as Region[]).map((r) => (
          <RegionTab
            key={r}
            region={r}
            label={`${REGION_CONFIG[r].icon} ${REGION_CONFIG[r].label}`}
      ...
      <MobileFilterToggle>
        <FilterSection title="Tier">
          {(Object.keys(TIER_CONFIG) as Tier[]).map((t) => (
            <FilterItem
              key={t}
              label={`${t}-${TIER_CONFIG[t].label}`}
              paramKey="tier"
              paramValue={t}
              active={activeTiers?.includes(t) || false}
              params={params}
            />
          ))}
        </FilterSection>
        <FilterSection title="Category">
          {(Object.keys(CATEGORY_CONFIG) as Category[]).map((cat) => (
            <FilterItem
              ...
              paramKey="category"
              paramValue={cat}
              active={activeCategories?.includes(cat) || false}
              params={params}
            />
          ))}
        </FilterSection>
        <FilterSection title="Status">
          {(Object.keys(STATUS_CONFIG) as Status[]).map((s) => (
            <FilterItem
              ...
              paramKey="status"
              paramValue={s}
              active={activeStatuses?.includes(s) || false}
              params={params}
            />
          ))}
        </FilterSection>
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1">
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm text-zinc-400">{filtered.length} competitions</p>
            <SortSelect current={sort} params={params} />
          </div>
          {filtered.length === 0 ? (
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-12 text-center">
              <p className="text-zinc-400">No competitions match your filters.</p>
            </div>
          ) : (
